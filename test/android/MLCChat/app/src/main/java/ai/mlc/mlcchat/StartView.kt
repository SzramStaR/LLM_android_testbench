    ...
    Column(
        ...
        Row(
            ...
            if (modelState.modelInitState.value == ModelInitState.Paused) {
                ...
                IconButton(
                    onClick = {
                        appViewModel.runBenchmark(
                            modelState = modelState,
                            userId = appViewModel.getDeviceId(),
                            model = modelState.modelConfig.modelId
                        )
                    },
                    enabled = appViewModel.chatState.interruptable() && !appViewModel.chatState.isBenchmarking.value,
                    modifier = Modifier
                        .aspectRatio(1f)
                        .weight(1f)
                        .padding(4.dp)
                    ...
                    Icon(
                        ...
                        contentDescription = "benchmark model",
                    )
                }
            } else {
                IconButton(
        ...
        if (appViewModel.chatState.isBenchmarking.value && appViewModel.chatState.modelName.value == modelState.modelConfig.modelId) {
            Row(
                ...
                LinearProgressIndicator(
                    progress = appViewModel.chatState.benchmarkRunCount.value.toFloat() / (appViewModel.chatState.benchmarkRunTotal.value.coerceAtLeast(1)),
                    modifier = Modifier
                        .weight(1f)
                        .height(4.dp)
                        .padding(end = 8.dp)
                ...
                Text(
                    text = "Benchmarking ${appViewModel.chatState.benchmarkRunCount.value}/${appViewModel.chatState.benchmarkRunTotal.value}",
